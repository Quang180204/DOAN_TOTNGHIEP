...
export default function LaptopPage() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/products/laptop?page=${page}&sortOrder=${sortOrder}`);
        if (res.data.success) {
        ...
        toast.error('Không thể tải danh sách laptop');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-6">
        <h1 className="text-2xl font-bold mb-4 md:mb-0">
          Laptop <span className="text-yellow-600">({total} sản phẩm)</span>
        </h1>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          <h2 className="text-xl text-gray-600">Chưa có sản phẩm laptop</h2>
        </div>
      ) : (
        <>
