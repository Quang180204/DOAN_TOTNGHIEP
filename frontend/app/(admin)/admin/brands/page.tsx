...
export default function BrandsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (editingBrand) {
        await api.put(`/admin/brands/edit/${editingBrand.brand_id}`, { brandName });
        toast.success('Cập nhật thương hiệu thành công');
      } else {
        await api.post('/admin/brands/create', { brandName });
      ...
      toast.error(editingBrand ? 'Cập nhật thất bại' : 'Thêm thất bại');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/admin/brands/delete/${selectedBrand.brand_id}`);
    ...
    <div className="px-4 py-6 w-full max-w-[1200px] mx-auto fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 bg-white px-6 py-4 rounded-2xl shadow-sm border border-gray-100">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-gray-900 tracking-tight">Thương hiệu</h1>
            <div className="flex items-center text-xs text-gray-500 space-x-2 mt-1">
              <Link href="/admin" className="cursor-pointer hover:text-blue-600 transition-colors">Bảng điều khiển</Link>
              <span className="text-gray-300">/</span>
              <span className="font-semibold text-gray-700">Danh sách thương hiệu</span>
            </div>
          </div>
        </div>
          ...
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-50/80 text-xs text-gray-500 uppercase tracking-wider">
              <tr>
                ...
                <th className="px-6 py-5 font-semibold text-gray-500">Tên thương hiệu</th>
              ...
              {brands.map((brand) => (
                <tr key={brand.brand_id} className="hover:bg-blue-50/30 transition-colors group">
                  <td className="px-6 py-4">
                    <span className="font-semibold text-gray-500">#{brand.brand_id}</span>
                  </td>
                  <td className="px-6 py-4">
                    <span className="font-bold text-gray-800 text-base">{brand.brand_name}</span>
                  </td>
                  <td className="px-6 py-4 text-center">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-gray-100 text-gray-700 border border-gray-200">
                      {brand.create_by || 'Admin'}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-center font-medium text-gray-500">
                    {formatDate(brand.create_at)}
                  </td>
                  <td className="px-6 py-4 text-center">
                    <ActionMenu
                      items={[
                  ...
                  <td colSpan={5} className="px-6 py-16 text-center">
                    <div className="flex flex-col items-center justify-center">
                      <div className="w-16 h-16 bg-gray-50 rounded-full border border-gray-100 flex items-center justify-center mb-3">
                        <i className="bi bi-tags text-2xl text-gray-300"></i>
                      </div>
                      <p className="text-gray-500 font-medium text-lg">Không tìm thấy thương hiệu nào</p>
                    </div>
                  </td>
                </tr>
              )}
      ...
      <ConfirmModal
        ...
        message={`Bạn có chắc chắn muốn xóa thương hiệu "<strong class="text-red-600">${selectedBrand?.brand_name}</strong>"?`}
        type="danger"
